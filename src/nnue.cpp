...
static inline uint8_t crelu_qa(int16_t x) {
    if (x <= 0)
        return 0;
    if (x >= QA)
        return static_cast<uint8_t>(QA);
    return static_cast<uint8_t>(x);
}

static int evaluate_from_accs(const int16_t acc_stm[HIDDEN],
                              const int16_t acc_ntm[HIDDEN]) {
    std::array<uint8_t, HIDDEN * 2> hidden{};

    for (int i = 0; i < HIDDEN; ++i) {
        hidden[size_t(i)] = crelu_qa(acc_stm[i]);
        hidden[size_t(HIDDEN + i)] = crelu_qa(acc_ntm[i]);
    }

    const int32_t sum = throne_simd::dot_u8_i8(hidden.data(), g_W2.data(), hidden.size());
    const float out = g_b2 + float(sum) / float(QA * QB);
    return int(std::lround(out * NETWORK_SCALE));
}
...
bool init(const std::string& filepath) {
    ...
    if (!read_all(f, g_W1.data(), g_W1.size() * sizeof(int16_t)) ||
        ...
        !read_all(f, g_W2.data(), g_W2.size() * sizeof(int8_t)) ||
